...
class ReconstructionParameters(ace.AceParameters):
    ...
    def __init__(self, prefix=None, suffix=None):
        ...
        doc = (
            "\t Intensity-transformation modes are 'identity', 'normalization_to_u8',\n"
        )
        ...
        self.registration.append(
            common.RegistrationParameters(prefix=[self._prefix, "linear_registration_"])
        )
        ...
        self.registration.append(
            common.RegistrationParameters(
                prefix=[self._prefix, "nonlinear_registration_"]
            )
        )
    ...
    prenormalized_suffix = property(
        _get_prenormalized_suffix, _set_prenormalized_suffix
    )
    ...
    def print_parameters(self):
        ...
        self.varprint(
            "intensity_sigma", self.intensity_sigma, self.doc["intensity_sigma"]
        )
        ...
        self.varprint(
            "keep_reconstruction",
            self.keep_reconstruction,
            self.doc["keep_reconstruction"],
        )
    ...
    def update_from_parameters(self, parameters):
        ...
        self.intensity_prenormalization = self.read_parameter(
            parameters, "intensity_prenormalization", self.intensity_prenormalization
        )
        self.prenormalization_min_percentile = self.read_parameter(
            parameters,
            "prenormalization_min_percentile",
            self.prenormalization_min_percentile,
        )
        self.prenormalization_max_percentile = self.read_parameter(
            parameters,
            "prenormalization_max_percentile",
            self.prenormalization_max_percentile,
        )
        ...
        self.intensity_transformation = self.read_parameter(
            parameters, "intensity_transformation", self.intensity_transformation
        )
        self.intensity_enhancement = self.read_parameter(
            parameters, "intensity_enhancement", self.intensity_enhancement
        )
        self.outer_contour_enhancement = self.read_parameter(
            parameters, "outer_contour_enhancement", self.outer_contour_enhancement
        )
        self.reconstruction_images_combination = self.read_parameter(
            parameters,
            "reconstruction_images_combination",
            self.reconstruction_images_combination,
        )
        ...
        #
        self.cell_normalization_min_method = self.read_parameter(
            parameters,
            "cell_normalization_min_method",
            self.cell_normalization_min_method,
        )
        self.cell_normalization_max_method = self.read_parameter(
            parameters,
            "cell_normalization_max_method",
            self.cell_normalization_max_method,
        )
        self.normalization_min_percentile = self.read_parameter(
            parameters,
            "normalization_min_percentile",
            self.normalization_min_percentile,
        )
        self.normalization_max_percentile = self.read_parameter(
            parameters,
            "normalization_max_percentile",
            self.normalization_max_percentile,
        )
        self.cell_normalization_sigma = self.read_parameter(
            parameters, "cell_normalization_sigma", self.cell_normalization_sigma
        )
        self.intensity_sigma = self.read_parameter(
            parameters, "intensity_sigma", self.intensity_sigma
        )
        #
        #
        #
        ...
        #
        #
        self.keep_reconstruction = self.read_parameter(
            parameters, "keep_reconstruction", self.keep_reconstruction
        )
    ...
    def is_prenormalization_equal(self, p):
        ...
        if self.intensity_prenormalization == p.intensity_prenormalization:
            ...
            if (
                self.prenormalization_min_percentile
                == p.prenormalization_min_percentile
                and self.prenormalization_max_percentile
                == p.prenormalization_max_percentile
            ):
                return True
            return False
        return False
    ...
    def is_normalization_equal(self, p):
        ...
        if self.intensity_transformation == p.intensity_transformation:
            if self.intensity_transformation.lower() == "identity":
                return True
            if (
                self.intensity_transformation.lower() == "normalization_to_u8"
            ...
            if self.intensity_transformation.lower() == "cell_normalization_to_u8":
                if (
                    self.cell_normalization_min_method
                    == p.cell_normalization_min_method
                    and self.cell_normalization_max_method
                    == p.cell_normalization_max_method
                    and self.cell_normalization_sigma == p.cell_normalization_sigma
                    and self.prenormalization_min_percentile
                    == p.prenormalization_min_percentile
                    and self.prenormalization_max_percentile
                    == p.prenormalization_max_percentile
                ):
    ...
    def is_intensity_enhancement_equal(self, p):
        if self.intensity_enhancement == p.intensity_enhancement:
            if (
                self.intensity_enhancement is None
                or self.intensity_enhancement.lower() == "none"
            ):
                return True
...
def get_deformation_from_current_to_previous(
    current_time, experiment, parameters, previous_time
):
    """
    ...
    current_image = common.find_file(
    ...
    previous_image = common.find_file(
        experiment.fusion_dir.get_directory(),
        previous_name,
        file_type="image",
        callfrom=proc,
    ...
    #
    affine_image = common.add_suffix(
        previous_image,
        "_affine",
        new_dirname=experiment.working_dir.get_tmp_directory(0),
        new_extension=experiment.default_image_suffix,
    )
    affine_trsf = common.add_suffix(
        previous_image,
        "_affine",
        new_dirname=experiment.working_dir.get_tmp_directory(0),
        new_extension="trsf",
    )
    vector_image = common.add_suffix(
        previous_image,
        "_vector",
        new_dirname=experiment.working_dir.get_tmp_directory(0),
        new_extension=experiment.default_image_suffix,
    )
    vector_trsf = common.add_suffix(
        previous_image,
        "_vector",
        new_dirname=experiment.working_dir.get_tmp_directory(0),
        new_extension="trsf",
    ...
    # linear registration
    #
    common.blockmatching(
        current_image,
        previous_image,
        affine_image,
        affine_trsf,
        None,
        parameters.registration[0],
    )
    ...
    if not os.path.isfile(affine_image) or not os.path.isfile(affine_trsf):
        monitoring.to_log_and_console(
            "    .. "
            + proc
            + " linear registration failed for time "
            + str(current_time),
            2,
        )
        return None
    ...
    if not os.path.isfile(vector_image) or not os.path.isfile(vector_trsf):
        monitoring.to_log_and_console(
            "    .. "
            + proc
            + " non-linear registration failed for time "
            + str(current_time),
            2,
        )
        return None
...
def get_previous_deformed_segmentation(
    current_time, experiment, parameters, previous_time=None
):
    """
    ...
    if prev_segimage is None:
        if previous_time is None:
            monitoring.to_log_and_console(
                "    .. " + proc + ": was called with 'previous_time = None'", 2
            )
        else:
            monitoring.to_log_and_console(
                "    .. "
    ...
    prev_def_segimage = common.add_suffix(
        prev_segimage,
        "_deformed",
        new_dirname=experiment.working_dir.get_tmp_directory(0),
        new_extension=experiment.default_image_suffix,
    ...
    deformation = get_deformation_from_current_to_previous(
        current_time, experiment, parameters, previous_time
    )
    ...
    if deformation is None:
        monitoring.to_log_and_console(
            "    .. "
            + proc
            + ": no deformation was found for time "
            + str(current_time)
            + " towards "
            + str(previous_time),
            2,
    ...
    monitoring.to_log_and_console(
        "    .. resampling of '" + str(prev_segimage).split(os.path.sep)[-1] + "'", 2
    )
...
def build_reconstructed_image(current_time, experiment, parameters, previous_time=None):
    ...
    reconstructed_image = common.add_suffix(
        input_image,
    ...
    if (
        os.path.isfile(reconstructed_image)
        and monitoring.forceResultsToBeBuilt is False
    ):
        monitoring.to_log_and_console(
            "    .. reconstructed image is '"
            + str(reconstructed_image).split(os.path.sep)[-1]
            + "'",
            2,
        )
    ...
    if parameters.intensity_prenormalization.lower() == "identity":
        ...
        prenormalized_image = common.add_suffix(
            input_image,
            local_suffix,
            new_dirname=experiment.working_dir.get_tmp_directory(0),
            new_extension=experiment.default_image_suffix,
        ...
        if (
            os.path.isfile(prenormalized_image)
            and monitoring.forceResultsToBeBuilt is False
        ):
            monitoring.to_log_and_console(
                "    .. prenormalization image is '"
                + str(prenormalized_image).split(os.path.sep)[-1]
                + "'",
                2,
            )
            input_image = prenormalized_image
        elif (
            parameters.intensity_prenormalization.lower() == "normalization_to_u8"
            or parameters.intensity_prenormalization.lower()
            == "global_normalization_to_u8"
        ):
            monitoring.to_log_and_console(
                "    .. intensity global prenormalization of '"
                + str(input_image).split(os.path.sep)[-1]
                + "'",
                2,
            )
            cpp_wrapping.global_intensity_normalization(
                input_image,
                prenormalized_image,
            ...
            parameters.intensity_prenormalization.lower() == "normalization_to_u16"
            or parameters.intensity_prenormalization.lower()
            == "global_normalization_to_u16"
        ):
            monitoring.to_log_and_console(
                "    .. intensity global prenormalization of '"
                + str(input_image).split(os.path.sep)[-1]
            ...
            monitoring.to_log_and_console(
                "    unknown intensity prenormalization method: '"
                + str(parameters.intensity_prenormalization)
                + "'",
                2,
            )
    ...
    if (
        parameters.intensity_transformation is None
        or parameters.intensity_transformation.lower() == "none"
    ):
        normalized_image = None
    elif parameters.intensity_transformation.lower() == "identity":
        normalized_image = input_image
    else:
        normalized_image = common.add_suffix(
            input_image,
            parameters.normalized_suffix + "_normalized",
            new_dirname=experiment.working_dir.get_tmp_directory(0),
            new_extension=experiment.default_image_suffix,
        )
        if (
            os.path.isfile(normalized_image)
            and monitoring.forceResultsToBeBuilt is False
        ):
            monitoring.to_log_and_console(
                "    .. normalization image is '"
                + str(normalized_image).split(os.path.sep)[-1]
                + "'",
                2,
            )
            #
            ...
            #
            if (
                parameters.intensity_transformation.lower() == "normalization_to_u8"
                or parameters.intensity_transformation.lower()
                == "global_normalization_to_u8"
                or parameters.intensity_transformation.lower()
        ...
        #
            ...
            intensity_image_bytes = 1
            monitoring.to_log_and_console(
                "    .. intensity global normalization of '"
                + str(input_image).split(os.path.sep)[-1]
                + "'",
                2,
            )
        ...
        #
            ...
            intensity_image_bytes = 1
            monitoring.to_log_and_console(
                "    .. intensity cell-based normalization of '"
                + str(input_image).split(os.path.sep)[-1]
                + "'",
                2,
            )
            if previous_time is None:
                monitoring.to_log_and_console(
                    "       previous time point was not given", 2
                )
                monitoring.to_log_and_console(
                    "    .. " + proc + ": switch to 'global normalization' ", 1
                )
                cpp_wrapping.global_intensity_normalization(
                    input_image,
                    normalized_image,
                ...
                previous_deformed_segmentation = get_previous_deformed_segmentation(
                    current_time, experiment, parameters, previous_time
                )
                cpp_wrapping.cell_intensity_normalization(
                    input_image,
                    previous_deformed_segmentation,
                    normalized_image,
                    min_percentile=parameters.normalization_min_percentile,
                    max_percentile=parameters.normalization_max_percentile,
        ...
        #
            ...
            parameters.intensity_transformation.lower() == "normalization_to_u16"
            or parameters.intensity_transformation.lower()
            == "global_normalization_to_u16"
        ):
            intensity_image_bytes = 2
            ...
            monitoring.to_log_and_console(
                "    unknown intensity transformation method: '"
                + str(parameters.intensity_transformation)
                + "'",
                2,
            )
            normalized_image = input_image
    ...
    if normalized_image is None:
        ...
        intensity_image = common.add_suffix(
            input_image,
            parameters.intensity_suffix + "_intensity",
            new_dirname=experiment.working_dir.get_tmp_directory(0),
            new_extension=experiment.default_image_suffix,
        )
        if (
            os.path.isfile(intensity_image)
            and monitoring.forceResultsToBeBuilt is False
        ):
            monitoring.to_log_and_console(
                "    .. smoothed intensity transformed image is '"
            ...
            monitoring.to_log_and_console(
                ...
                + str(normalized_image).split(os.path.sep)[-1]
                + "' with sigma = "
                + str(parameters.intensity_sigma),
                2,
            )
            ...
            if not os.path.isfile(intensity_image):
                monitoring.to_log_and_console(
                    "    .. "
                    + proc
                    + ": error when smoothing intensity transformed image"
                )
    ...
    #
    if (
        parameters.intensity_enhancement is None
        or parameters.intensity_enhancement.lower() == "none"
    ):
        enhanced_image = None
    else:
        ...
        #
        enhanced_name = common.add_suffix(
            str(input_image).split(os.path.sep)[-1], local_suffix
        )
        enhanced_image = common.find_file(
            experiment.working_dir.get_tmp_directory(0),
            enhanced_name,
            file_type="image",
            callfrom=proc,
            local_monitoring=None,
        ...
        if enhanced_image is not None and monitoring.forceResultsToBeBuilt is False:
            enhanced_image = os.path.join(
                experiment.working_dir.get_tmp_directory(0), enhanced_image
            )
            monitoring.to_log_and_console(
                "    .. intensity enhanced image is '"
                + str(enhanced_image).split(os.path.sep)[-1]
                + "'",
                2,
            )
        else:
            enhanced_image = common.add_suffix(
                input_image,
                local_suffix,
                new_dirname=experiment.working_dir.get_tmp_directory(0),
                new_extension=experiment.default_image_suffix,
            ...
            if (
                parameters.intensity_enhancement.lower() == "gace"
                or parameters.intensity_enhancement.lower() == "ace"
            ):
                monitoring.to_log_and_console(
                ...
                if parameters.membrane_seg is None:
                    ace.global_membrane_enhancement(
                        input_image,
                        enhanced_image,
                        experiment,
                ...
                monitoring.to_log_and_console(
                    ...
                    + str(parameters.intensity_enhancement)
                    + "'",
                    2,
                )
    ...
    # added searching for contour images in <PATH_EMBRYO>/CONTOUR/CONTOUR_<EXP_CONTOUR>/ - Gesa
        ...
        contour_image = common.find_file(
            contour_dir,
            contour_name,
            file_type="image",
            callfrom=proc,
        ...
        monitoring.to_log_and_console(
            f"    .. getting contour image from: '" + contour_image + "'", 2
        )
    ...
    ######
        ...
        contour_image = common.add_suffix(
            input_image,
            local_suffix,
            new_dirname=experiment.working_dir.get_tmp_directory(0),
            new_extension=experiment.default_image_suffix,
        ...
        if os.path.isfile(contour_image) and monitoring.forceResultsToBeBuilt is False:
            monitoring.to_log_and_console(
                "    .. outer contour image is '"
                + str(contour_image).split(os.path.sep)[-1]
                + "'",
                2,
            )
        else:
            ...
            cpp_wrapping.outer_contour(
                deformed_segmentation, contour_image, monitoring=monitoring
            )
    ...
    if intensity_image is not None and enhanced_image is None and contour_image is None:
        ...
        intensity_image is None and enhanced_image is not None and contour_image is None
    ):
        if parameters.keep_reconstruction:
            experiment.working_dir.make_rec_directory()
        ...
        intensity_image is None and enhanced_image is None and contour_image is not None
    ):
        monitoring.to_log_and_console(
            "    weird, only the outer contour is used for reconstruction", 2
        )
        if parameters.keep_reconstruction:
            experiment.working_dir.make_rec_directory()
    ...
    if (
        parameters.reconstruction_images_combination.lower() == "maximum"
        or parameters.reconstruction_images_combination.lower() == "max"
    ):
        arit_options = " -max"
    elif (
        parameters.reconstruction_images_combination.lower() == "addition"
        ...
        monitoring.to_log_and_console(
            "    unknown image combination operation: '"
            + str(parameters.reconstruction_images_combination)
            + "'",
            2,
        )
        return None
    ...
    if intensity_image is not None:
        if enhanced_image is not None:
            if (
                ...
                if intensity_image_bytes == 1 and enhanced_image_bytes == 1:
                    ...
                    arit_options += " -o 2"
            cpp_wrapping.arithmetic_operation(
                intensity_image,
                enhanced_image,
                reconstructed_image,
            ...
            if contour_image is not None:
                if (
                    ...
                    if (
                        intensity_image_bytes == 1
                        and enhanced_image_bytes == 1
                        and contour_image_bytes == 1
                    ):
                        arit_options += " -o 1"
                    else:
                        arit_options += " -o 2"
            ...
            if contour_image is not None:
                ...
                monitoring.to_log_and_console(
                    "    weird. There should be a fusion to be done. Returns intensity",
                    2,
                )
                return intensity_image
        ...
        if contour_image is not None:
            ...
            monitoring.to_log_and_console(
                "    weird. There should be a fusion to be done. Returns enhanced", 2
            )
            return enhanced_image
    elif contour_image is not None:
        monitoring.to_log_and_console(
            "    weird. There should be a fusion to be done. Returns contours", 2
        )
        return contour_image
