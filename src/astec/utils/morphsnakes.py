...
class MorphACWE(object):
    ...
    levelset = property(
        lambda self: self._u, set_levelset, doc="The level set embedding function (u)."
    )
    ...
    def step(self):
        ...
        aux = abs_dres * (
            self.lambda1 * (data - c1) ** 2 - self.lambda2 * (data - c0) ** 2
        )
...
class MorphGAC(object):
    ...
    threshold = property(
        lambda self: self._theta, set_threshold, doc="The threshold value (θ)."
    )
