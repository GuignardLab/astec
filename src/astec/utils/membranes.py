...
def calculate_incremented_bounding_boxes(watershed_labels, increment=1):
    ...
    for i, b_box in enumerate(b_boxes):
        if b_box is not None:
            new_entry = (
                slice(
                    max(b_box[0].start - increment, 0),
                    min(b_box[0].stop + increment, z_max - 1),
                    b_box[0].step,
                ),
                slice(
                    max(b_box[1].start - increment, 0),
                    min(b_box[1].stop + increment, x_max - 1),
                    b_box[1].step,
                ),
                slice(
                    max(b_box[2].start - increment, 0),
                    min(b_box[2].stop + increment, y_max - 1),
                    b_box[2].step,
                ),
            )
...
def extract_touching_surfaces(watershed_labels):
    ...
    for label in unique_labels:
        ...
        for neighbor in neighbors:
            ...
            if neighbor > label:
                ...
                if (n_mask.sum() > 0) and (
                    tuple(sorted([label, neighbor])) not in mapp_mem.keys()
                ):
    ...
    if len(lost_membranes) > 0:
        mapp_mem = {
            pair: mem_id
            for pair, mem_id in mapp_mem.items()
            if mem_id in set(final_membranes)
        }
...
def volume_ratio_after_closing(interface_image, mapper, voxelsize, iterations=1):
    ...
    incremented_b_boxes = calculate_incremented_bounding_boxes(
        interface_image, increment=increment
    )
    ...
    for label in mapper.values():
        ...
        interface_closed = nd_binary_closing(
            interface, structure=structure, iterations=iterations
        )
...
def merge_labels_with_false_membranes(
    false_pairs_list, original_watershed_labels, selected_seeds, correspondences
):
    """
    ...
    for cell_set in cc_list:
        list_of_mothers = [
            ...
            if len(set(value).intersection(cell_set)) > 0
        ]
        ...
        for label in cell_set:
            mother = [key for key, value in correspondences.items() if label in value][
                0
            ]
...
def translate_cell_pair_to_previous(cell_pair, reversed_correspondences):
    return tuple(
        sorted(
            [
                reversed_correspondences[cell_pair[0]],
                reversed_correspondences[cell_pair[1]],
            ]
        )
    )
        ...
        if mother_cell in changed_cells:
            ...
            if new_daughter not in processed_daughters:
                ...
                cut_correspondences = {
                    key: value
                    for key, value in correspondences.items()
                    if new_daughter in value
                }
                if (
                    len(cut_correspondences) == 1
                ):  # because if there is just one, the necessary change was already doine during the merging
                    new_correspondences[mother_cell] = correspondences[mother_cell]
                else:
                    len_daughters = [
                        len(daughter_list)
                        for daughter_list in cut_correspondences.values()
                    ]
                    ...
                    if max(len_daughters) == 1 and len(len_daughters) > 1:
                        ...
                        max(len_daughters) == 2
                    ):  # that means there must be cells that have divided
                        # is there more than one daughter pair?
                        daughter_pairs = [
                            x for x in cut_correspondences.values() if len(x) == 2
                        ]
                        if (
                            len(daughter_pairs) > 1
                        ):  # if there is just one, the necessary changes have already been done in the merging
                            for i, daughter_pair_list in enumerate(daughter_pairs):
                                mother = [
                                    key
                                    for key, value in correspondences.items()
                                    if value == daughter_pair_list
                                ][0]
