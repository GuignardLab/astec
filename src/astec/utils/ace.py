...
class AceParameters(common.PrefixedParameter):
    ...
    def print_parameters(self):
        ...
        self.varprint(
            "hard_thresholding", self.hard_thresholding, self.doc["hard_thresholding"]
        )
        ...
        self.varprint(
            "sample_random_seed",
            self.sample_random_seed,
            self.doc["sample_random_seed"],
        )

        self.varprint(
            "bounding_box_dilation",
            self.bounding_box_dilation,
            self.doc["bounding_box_dilation"],
        )
    ...
    def write_parameters_in_file(self, logfile):
        ...
        self.varwrite(
            logfile, "sigma_membrane", self.sigma_membrane, self.doc["sigma_membrane"]
        )

        self.varwrite(
            logfile,
            "hard_thresholding",
            self.hard_thresholding,
            self.doc["hard_thresholding"],
        )
        self.varwrite(
            logfile, "hard_threshold", self.hard_threshold, self.doc["hard_threshold"]
        )

        self.varwrite(logfile, "manual", self.manual, self.doc["manual"])
        self.varwrite(
            logfile, "manual_sigma", self.manual_sigma, self.doc["manual_sigma"]
        )
    ...
    def update_from_parameters(self, parameters):
        self.sigma_membrane = self.read_parameter(
            parameters, "sigma_membrane", self.sigma_membrane
        )

        self.hard_thresholding = self.read_parameter(
            parameters, "hard_thresholding", self.hard_thresholding
        )
        self.hard_threshold = self.read_parameter(
            parameters, "hard_threshold", self.hard_threshold
        )

        self.manual = self.read_parameter(parameters, "manual", self.manual)
        self.manual_sigma = self.read_parameter(
            parameters, "manual_sigma", self.manual_sigma
        )
        self.sensitivity = self.read_parameter(
            parameters, "sensitivity", self.sensitivity
        )
        ...
        self.sample_random_seed = self.read_parameter(
            parameters, "sample_random_seed", self.sample_random_seed
        )

        self.bounding_box_dilation = self.read_parameter(
            parameters, "bounding_box_dilation", self.bounding_box_dilation
        )

        self.processors = self.read_parameter(parameters, "processors", self.processors)

        self.membrane_seg = self.read_parameter(
            parameters, "membrane_seg", self.membrane_seg
        )
...
def global_membrane_enhancement(
    path_input,
    path_output,
    experiment,
    binary_input=False,
    ...
    if binary_input is False:
        ...
        if (
            ext_image is None
            or phi_image is None
            or theta_image is None
            or monitoring.forceResultsToBeBuilt is True
        ):
            monitoring.to_log_and_console(
                "       membrane extraction of '"
            ...
            cpp_wrapping.membrane_extraction(
                path_input,
                tmp_prefix_name,
                scale=parameters.sigma_membrane,
        ...
        if bin_image is None or monitoring.forceResultsToBeBuilt is True:
            if bin_image is None:
                bin_image = os.path.join(
                    temporary_path,
                    prefix_name + ".bin." + experiment.default_image_suffix,
                )
            if parameters.hard_thresholding is True:
                ...
                #
                monitoring.to_log_and_console(
                    "       membrane hard thresholding of '"
                    + str(ext_image).split(os.path.sep)[-1]
                    + "'",
                    2,
                )
                cpp_wrapping.seuillage(
                    path_input=ext_image,
                    path_output=bin_image,
                    low_threshold=parameters.hard_threshold,
                    monitoring=monitoring,
                )
            else:
                #
                ...
                #
                monitoring.to_log_and_console(
                    "       membrane anisotropic thresholding of '"
                    + str(ext_image).split(os.path.sep)[-1]
                    + "'",
                    2,
                )
                cpp_wrapping.anisotropic_histogram(
                    path_input_extrema=ext_image,
                    path_output_histogram=hist_file,
                    path_output=bin_image,
    ...
    monitoring.to_log_and_console(
        "       tensor voting from '" + str(path_tv_input).split(os.path.sep)[-1] + "'",
        2,
    )
    ...
    if not os.path.isfile(path_output) or monitoring.forceResultsToBeBuilt is True:
        cpp_wrapping.tensor_voting_membrane(
            path_tv_input,
            bin_prefix_name,
            path_output,
            ...
            sigma_smoothing=parameters.sigma_LF,
            monitoring=monitoring,
        )
...
def cell_binarization(parameters_for_parallelism):
    ...
    cpp_wrapping.crop_image(
        previous_deformed_segmentation, cell_mask, bbox, monitoring=monitoring
    )
    cpp_wrapping.seuillage(
        cell_mask,
        cell_mask,
        low_threshold=label_of_interest,
        high_threshold=label_of_interest,
        monitoring=monitoring,
    )
    ...
    cpp_wrapping.mathematical_morphology(
        cell_mask,
        cell_mask,
        other_options=" -dil -R " + str(dilation_radius),
        monitoring=monitoring,
    )
    ...
    if parameters.hard_thresholding is True:
        ...
        cpp_wrapping.anisotropic_histogram(
            path_input_extrema=cell_ext,
            path_output_histogram=cell_hist,
            path_output=cell_bin,
            path_input_mask=cell_mask,
            manual=parameters.manual,
            manual_sigma=parameters.manual_sigma,
            sensitivity=parameters.sensitivity,
            monitoring=monitoring,
    ...
    cpp_wrapping.logical_operation(
        cell_mask, cell_bin, cell_bin, other_options="-mask", monitoring=monitoring
    )
...
def cell_membrane_enhancement(
    ...
    #
    path_bboxes = common.add_suffix(
        previous_deformed_segmentation,
        "_bounding_boxes",
        new_dirname=temporary_path,
        new_extension="txt",
    )
    bboxes = cpp_wrapping.bounding_boxes(
        previous_deformed_segmentation, path_bboxes=path_bboxes
    )
    ...
    monitoring.to_log_and_console(
        "       ----- start: cell-based parallel binarization -----", 3
    )
    ...
    for label_of_interest in labels_of_interest:
        monitoring.to_log_and_console(
            "       membrane binarization of cell '" + str(label_of_interest) + "'", 3
        )

        parameters_for_parallelism = (
            label_of_interest,
            bboxes[label_of_interest],
            previous_deformed_segmentation,
            tmp_prefix_name,
            parameters,
    ...
    monitoring.to_log_and_console(
        "       ----- end:   cell-based parallel binarization -----", 3
    )
    monitoring.to_log_and_console(" ", 3)
    ...
    cpp_wrapping.create_image(
        bin_image,
        tmp_prefix_name + ".ext." + experiment.default_image_suffix,
        "-o 1",
        monitoring=monitoring,
    )
    ...
    for binary_output_cell in outputs:
        ...
        cpp_wrapping.patch_logical_operation(
            cell_bin, bin_image, bin_image, bbox, "-or", monitoring=monitoring
        )
        if not _instrumented_:
            os.remove(cell_bin)
...
def light_LACE(parameters):
    ...
    assert (
        (os.path.exists(path_membrane_prefix + ".ext.inr"))
        and (os.path.exists(path_membrane_prefix + ".theta.inr"))
        and (os.path.exists(path_membrane_prefix + ".phi.inr"))
    )
    ...
    if bbox:
        ...
        cpp_wrapping.obsolete_seuillage(
            path_mask_dil,
            path_output=path_mask_dil,
            sb=label_of_interest,
            sh=label_of_interest,
            verbose=verbose,
        )
    else:
        cpp_wrapping.obsolete_seuillage(
    ...
    if (
        True
    ):  # Computation of the dilation ray in real coordinates / Calcul du rayon de dilatation en coordonnees reelles
        rayon_dil /= imread(path_mask).voxelsize[0]
        rayon_dil = int(rayon_dil + 0.5)
    cpp_wrapping.obsolete_morpho(
        path_mask_dil, path_mask_dil, " -dil -R " + str(rayon_dil), verbose=verbose
    )
    ...
    if bbox:
        cpp_wrapping.obsolete_cropImage(
            path_membrane_prefix + ".ext.inr", path_ext_tmp, bbox, verbose=verbose
        )
    ...
    if not hard_thresholding:
        ...
        if bbox:
            cpp_wrapping.obsolete_cropImage(
                path_membrane_prefix + ".theta.inr",
                path_theta_tmp,
                bbox,
                verbose=verbose,
            )
            cpp_wrapping.obsolete_cropImage(
                path_membrane_prefix + ".phi.inr", path_phi_tmp, bbox, verbose=verbose
            )
            cpp_wrapping.obsolete_anisotropicHist(
                path_input=path_ext_tmp,
                path_output=path_bin,
                path_mask=path_mask_dil,
                manual=manual,
            ...
            cpp_wrapping.obsolete_anisotropicHist(
                path_input=path_membrane_prefix + ".ext.inr",
                path_output=path_bin,
                path_mask=path_mask_dil,
                manual=manual,
                manual_sigma=manual_sigma,
    ...
    cpp_wrapping.obsolete_Logic(
        path_mask_dil, path_bin, path_bin, Mode="mask", verbose=verbose
    )
...
def LACE(
    ...
    assert (
        os.path.exists(path_fused_0)
        and os.path.exists(path_fused_1)
        and os.path.exists(path_seg_0)
    )
    ...
    apply_trsf(
        path_seg_0,
        path_trsf=path_vector,
        path_output=path_mask,
        template=path_fused_1,
        nearest=True,
        verbose=verbose,
    )

    if (
        (not os.path.exists(path_membrane_prefix + ".ext.inr"))
        or (not os.path.exists(path_membrane_prefix + ".theta.inr"))
        or (not os.path.exists(path_membrane_prefix + ".phi.inr"))
    ):
        ...
        cpp_wrapping.obsolete_membrane_renforcement(
            path_fused_1,
            prefix_output=path_membrane_prefix,
            path_mask=path_mask_dil,
            init=sigma_membrane,
            verbose=verbose,
        )
    ...
    if (
        not keep_tmp_bin
        and path_membrane_prefix
        and os.path.exists(path_membrane_prefix + ".bin.inr")
    ):
    ...
    if (
        not keep_hist
        and path_membrane_prefix
        and os.path.exists(path_membrane_prefix + ".hist.txt")
    ):
...
def GLACE(
    ...
    GLACE_from_resampled_segmentation(
        path_fused_1,
        path_seg_trsf,
        labels_of_interest=labels_of_interest,
        background=background,
        path_membrane_prefix=path_membrane_prefix,
        path_bin=path_bin,
...
def GLACE_from_resampled_segmentation(
    path_fused_1,
    path_seg_trsf,
    labels_of_interest="all",
    background=[0, 1],
    ...
    cpp_wrapping.obsolete_createImage(
        path_union_of_local_bins, path_fused_1, "-o 1", verbose=verbose
    )
    ...
    for path_local_bin in outputs:
        ...
        cpp_wrapping.obsolete_patchLogic(
            path_local_bin,
            path_union_of_local_bins,
            path_union_of_local_bins,
            bbox,
            Mode="or",
            verbose=verbose,
...
def GACE(
    ...
    if binary_input:
        ...
        if not hard_thresholding:
            ...
            cpp_wrapping.obsolete_anisotropicHist(
                path_input=path_membrane_prefix + ".ext.inr",
                path_output=path_membrane_prefix + ".bin.inr",
                path_mask=None,
                manual=manual,
                manual_sigma=manual_sigma,
                sensitivity=sensitivity,
        ...
        if path_bin and not os.path.exists(path_bin):
            ...
            cpp_wrapping.obsolete_copy(
                path_membrane_prefix + ".bin.inr", path_bin, verbose=verbose
            )
    ...
    if (not os.path.exists(".".join(binary_file_decomp) + ".theta.inr")) or (
        ...
        assert (
            len(binary_file_decomp) > 1
            and os.path.exists(".".join(binary_file_decomp[:-1]) + ".theta.inr")
            and os.path.exists(".".join(binary_file_decomp[:-1]) + ".phi.inr")
        ), (
            "Error : unexpectedly, <prefix>.theta.inr and/or <prefix>.phi.inr not found for file "
            + path_tv_input
            + " before tensor voting step"
        )

    cpp_wrapping.obsolete_TVmembrane(
        path_input=path_tv_input,
        path_output=path_TV,
        path_mask=None,
        scale=sigma_TV,
        sample=sample,
    ...
    if not binary_input and not keep_membrane:
        if path_membrane_prefix:
            ...
            files_to_rm += "".join(
                [
                    path_membrane_dir + os.path.sep + i + " "
                    for i in os.listdir(path_membrane_dir)
                    if os.path.isfile(path_membrane_dir + os.path.sep + i)
                    and (path_membrane_dir + os.path.sep + i).count(
                        path_membrane_prefix
                    )
                ]
            )
