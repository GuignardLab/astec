...
class PostCorrectionParameters(common.PrefixedParameter):
    ...
    def print_parameters(self):
        ...
        self.varprint(
            "test_branch_length",
            self.test_branch_length,
            self.doc["test_branch_length"],
        )
        self.varprint(
            "test_early_division",
            self.test_early_division,
            self.doc["test_early_division"],
        )
        self.varprint(
        ...
        self.varprint(
            ...
            self.doc["correlation_threshold"],
        )
    ...
    def update_from_parameters(self, parameters):
        ...
        self.volume_minimal_value = self.read_parameter(
            parameters, "Volume_Threshold", self.volume_minimal_value
        )

        self.lifespan_minimal_value = self.read_parameter(
            parameters, "lifespan_minimal_value", self.lifespan_minimal_value
        )

        self.test_branch_length = self.read_parameter(
            parameters, "test_branch_length", self.test_branch_length
        )
        self.test_early_division = self.read_parameter(
            parameters, "test_early_division", self.test_early_division
        )
        self.test_early_division = self.read_parameter(
            parameters, "soon", self.test_early_division
        )
        self.test_early_division = self.read_parameter(
            parameters, "Soon", self.test_early_division
        )

        self.test_volume_correlation = self.read_parameter(
            parameters, "test_volume_correlation", self.test_volume_correlation
        )
        self.correlation_threshold = self.read_parameter(
            parameters, "correlation_threshold", self.correlation_threshold
        )
        self.correlation_threshold = self.read_parameter(
            parameters, "pearson_threshold", self.correlation_threshold
        )
        self.correlation_threshold = self.read_parameter(
            parameters, "PearsonThreshold", self.correlation_threshold
        )

        self.test_postponing_division = self.read_parameter(
            parameters, "test_postponing_division", self.test_postponing_division
        )
        self.postponing_correlation_threshold = self.read_parameter(
            parameters,
            "postponing_correlation_threshold",
            self.postponing_correlation_threshold,
        )
        self.postponing_minimal_length = self.read_parameter(
            parameters, "postponing_minimal_length", self.postponing_minimal_length
        )
        self.postponing_window_length = self.read_parameter(
            parameters, "postponing_window_length", self.postponing_window_length
        )
...
def _map_cell_fusion(
    astec_image, post_image, current_time, cells_to_be_fused, time_digits_for_cell_id=4
):
    #
    ...
    if labels_to_be_fused == "":
        monitoring.to_log_and_console(
            "    no cell fusion to be done for time #" + str(current_time), 2
        )
...
def _test_early_division(
    ...
    #
    if (
        len(sister_branch) < lifespan_minimal_value
    ...
    #
    if (
        len(mother_branch) < lifespan_minimal_value
...
def _test_volume_correlation(
    direct_lineage, volume, division_cell, cell, correlation_threshold
):
    ...
    pearson_correlation = pearsonr(
        branch_volume[:min_length], sister_volume[:min_length]
    )
...
def _get_leaves(direct_lineage, reverse_lineage, volume, experiment, parameters):
    ...
    candidates_for_deletion = [
        ...
        for leaf in leaves
        if (
            (int(leaf / 10**time_digits_for_cell_id) < last_time)
    ...
    for leaf in candidates_for_deletion:
        ...
        if division_cell != "":
            ...
            branch.reverse()
            if (
                int(branch[0] / 10**time_digits_for_cell_id)
                > experiment.first_time_point
            ):
                if len(branch) <= 4:
                    monitoring.to_log_and_console(
                        "        ... found an orphan branch: " + str(branch), 3
                    )
                else:
                    monitoring.to_log_and_console(
                        "        ... found an orphan branch: "
                        + str(branch[0:2])
                        + "..."
                        + str(branch[-2:]),
                        3,
    ...
    #
    branches = list(
        zip(candidates_for_deletion, lengths, division_cells, division_times)
    )
    return branches


def _fuse_branch(
    lineage, volume, surfaces, labels_to_be_fused, division_cell, branch, experiment
):
    proc = "_fuse_branch"
    time_digits_for_cell_id = experiment.get_time_digits_for_cell_id()
    ...
    while True:
        ...
        labels_to_be_fused.setdefault(cell_time, []).append(
            (progeny[iprogeny], branch[ibranch])
        )
            ...
            for i in range(len(progeny)):
                if progeny[i] in surfaces[branch[ibranch]]:
                    if iprogeny == -1:
                        ...
                        surfaces[branch[ibranch]][progeny[i]]
                        > surfaces[branch[ibranch]][progeny[iprogeny]]
                    ):
                        iprogeny = i
            if iprogeny == -1:
                monitoring.to_log_and_console(
...
def _prune_lineage_tree(lineage, volume, surfaces, experiment, parameters):
    ...
    last_time = int(
        max(set([v for values in list(lineage.values()) for v in values]))
        / 10**time_digits_for_cell_id
    )
    ...
    while True:
        ...
        candidates = [
            (le, b, dc, dt)
            for le, b, dc, dt in branches
            if le not in undeletable_leaves
        ]
        ...
        for leaf, branch_length, division_cell, division_time in candidates:
            ...
            if (
                parameters.test_branch_length
                and branch_length < parameters.lifespan_minimal_value
            ):
                n_fusions_length += 1
            elif parameters.test_early_division and _test_early_division(
                lineage,
                reverse_lineage,
                division_cell,
                branch[0],
                parameters.lifespan_minimal_value,
                first_time,
                last_time,
                time_digits_for_cell_id,
            ):
                n_fusions_early_division += 1
            elif parameters.test_volume_correlation and _test_volume_correlation(
                lineage,
                volume,
                division_cell,
                branch[0],
        ...
        n_fusions = (
            n_fusions_length + n_fusions_early_division + n_fusions_volume_correlation
        )
        if n_fusions == 0:
            break
        monitoring.to_log_and_console(
            "         "
            + str(n_fusions)
            + " branches have been fused at division time "
            + str(iteration_division_time),
            1,
        )
    ...
    monitoring.to_log_and_console(
        "       - " + str(total_n_fusions_length) + " fusions because of short length",
        2,
    )
    monitoring.to_log_and_console(
        "       - "
        + str(total_n_fusions_early_division)
        + " fusions because of early division",
        2,
    )
    monitoring.to_log_and_console(
        "       - "
        + str(total_n_fusions_volume_correlation)
        + " fusions because of volume anti-correlation",
        2,
    )
    monitoring.to_log_and_console("", 2)
...
def _get_postponing_candidate_divisions(lineage, volume, parameters):
    ...
    for c in division_list:
        ...
        #
        if (
            pearsonr(vol0[:min_length], vol1[:min_length])[0]
            >= -parameters.postponing_correlation_threshold
        ):
            continue
    ...
    monitoring.to_log_and_console(
        "       found "
        + str(len(valid_division_list))
        + " candidates among "
        + str(len(division_list))
        + " divisions",
        3,
    )
...
def _get_postponing_scores(valid_division_list, lineage, volume, parameters):
    ...
    for c in valid_division_list:
        ...
        for i in range(0, min_length - window_length + 1):
            scores.append(
                pearsonr(vol0[i : i + window_length], vol1[i : i + window_length])[0]
            )
        scores_window[c] = np.array(scores)
    return scores_window


def _postpone_division(
    lineage, volume, surfaces, labels_to_be_fused, experiment, parameters
):
    ...
    #
    valid_division_list = _get_postponing_candidate_divisions(
        lineage, volume, parameters
    )
    if len(valid_division_list) == 0:
        return lineage, volume, surfaces, labels_to_be_fused
    ...
    #
    scores_window = _get_postponing_scores(
        valid_division_list, lineage, volume, parameters
    )
    ...
    for c, scores in scores_window.items():
        ...
        if len(sister0_branch) <= 4:
            ...
            str0 = (
                "["
                + str(sister0_branch[0])
                + ", "
                + str(sister0_branch[1])
                + ", ..., "
                + str(sister0_branch[-2])
                + ", "
                + str(sister0_branch[-1])
                + "]"
            )
        if len(sister1_branch) <= 4:
            ...
            str1 = (
                "["
                + str(sister1_branch[0])
                + ", "
                + str(sister1_branch[1])
                + ", ..., "
                + str(sister1_branch[-2])
                + ", "
                + str(sister1_branch[-1])
                + "]"
            )
    ...
    monitoring.to_log_and_console(
        "       - " + str(postponed_divisions) + " divisions were postponed", 2
    )
...
def contact_surface_computation(experiment, parameters):
    ...
    monitoring.to_log_and_console(
        "       will compute contact surface properties from '" + str(astec_dir) + "'",
        0,
    )
    ...
    #
    name_format = (
        experiment.astec_dir.get_file_prefix()
        + experiment.astec_dir.get_file_suffix()
        + experiment.astec_dir.get_time_prefix()
        + experiment.get_time_format()
    )

    suffix = common.get_file_suffix(
        experiment, astec_dir, name_format, flag_time=experiment.get_time_format()
    )
    if suffix is None:
        monitoring.to_log_and_console(
            proc + ": no consistent naming was found in '" + str(astec_dir) + "'", 1
        )
    ...
    cpp_wrapping.cell_properties(
        ...
        n_processors=parameters.processors,
...
def _check_volume_one_image(prop, image_name, current_time, time_digits_for_cell_id=4):
    ...
    labels_from_image = [
        current_time * 10**time_digits_for_cell_id + int(label)
        for label in labels_from_image
    ]
    volume_from_image = dict(list(zip(labels_from_image, volume)))

    labels_from_lineage = [
        label
        for label in list(volume_from_lineage.keys())
        if label / 10**time_digits_for_cell_id == current_time
    ]

    labels_in_lineage_not_in_image = list(
        set(labels_from_lineage).difference(set(labels_from_image))
    )
    labels_in_image_not_in_lineage = list(
        set(labels_from_image).difference(set(labels_from_lineage))
    )
    ...
    volume_error = [
        ...
        if volume_from_image[label] != volume_from_lineage[label]
    ]
    ...
    if len(labels_in_lineage_not_in_image) > 0:
        ...
        monitoring.to_log_and_console(
            "  - "
            + str(len(labels_in_lineage_not_in_image))
            + " cells present in volume dictionary not in image: ",
            1,
        )
        _print_list(
            prop,
            labels_in_lineage_not_in_image,
    ...
    if len(labels_in_image_not_in_lineage) > 0:
        ...
        monitoring.to_log_and_console(
            "  - "
            + str(len(labels_in_image_not_in_lineage))
            + " cells present in image not in volume dictionary: ",
            1,
        )
        _print_list(
            prop,
            labels_in_image_not_in_lineage,
            time_digits_for_cell_id=time_digits_for_cell_id,
        )
    ...
    if len(volume_error) > 0:
        ...
        monitoring.to_log_and_console(
            "  - "
            + str(len(volume_error))
            + " cells with different volume in image and volume dictionary: ",
            1,
        )
...
def postcorrection_process(experiment, parameters):
    ...
    if lineage_tree_file is None:
        monitoring.to_log_and_console(
            str(proc) + ": unable to find lineage file in " + str(segmentation_dir)
        )
        sys.exit(1)
        ...
        monitoring.to_log_and_console(
            str(proc) + ": " + str(lineage_tree_file) + " is not a file?"
        )
        sys.exit(1)
    ...
    #
    dict_lineage = ioproperties.get_dictionary_entry(
        lineage_tree_information, "lineage"
    )
    if dict_lineage == {}:
        monitoring.to_log_and_console(
            str(proc) + ": empty lineage information in " + str(lineage_tree_file)
        )
        sys.exit(1)
    ...
    if dict_volume == {}:
        monitoring.to_log_and_console(
            str(proc) + ": empty volume information in " + str(lineage_tree_file)
        )
        sys.exit(1)
    ...
    if dict_surface == {}:
        monitoring.to_log_and_console(
            str(proc) + ": empty surface information in " + str(surface_file)
        )
        sys.exit(1)
    ...
    lineage, volume, surfaces, cells_to_be_fused = _prune_lineage_tree(
        dict_lineage, dict_volume, dict_surface, experiment, parameters
    )
    ...
    if parameters.test_postponing_division:
        ...
        lineage, volume, surfaces, cells_to_be_fused = _postpone_division(
            lineage, volume, surfaces, cells_to_be_fused, experiment, parameters
        )

    #
    # save lineage tree
    #
    new_lineage_tree_information = {
        ioproperties.keydictionary["lineage"]["output_key"]: lineage,
    ...
    lineage_tree_path = os.path.join(
        segmentation_dir,
        experiment.post_dir.get_file_name("_lineage")
        + "."
    ...
    for current_time in range(first_time, last_time + 1, experiment.delta_time_point):
        acquisition_time = experiment.get_time_index(current_time)
        monitoring.to_log_and_console(
            "   ... cell fusion of time #" + acquisition_time, 1
        )
        ...
        if post_image is not None:
            if monitoring.forceResultsToBeBuilt is False:
                monitoring.to_log_and_console(
                    "       post-segmentation image already existing", 2
                )
                continue
            else:
                monitoring.to_log_and_console(
                    "       post-segmentation image already existing, but forced", 2
                )
        post_image = os.path.join(
            post_dir, post_name + "." + experiment.result_image_suffix
        )
    ...
    if parameters.lineage_diagnosis and False:
        monitoring.to_log_and_console(
            "   ... test lineage (after cell merging in image)", 1
        )
