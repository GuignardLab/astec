...
class MorphoSnakeParameters(common.PrefixedParameter):
    ...
    def update_from_parameters(self, parameters):
        self.dilation_iterations = self.read_parameter(
            parameters, "dilation_iterations", self.dilation_iterations
        )
        self.dilation_iterations = self.read_parameter(
            parameters, "astec_MorphosnakeIterations", self.dilation_iterations
        )

        self.iterations = self.read_parameter(parameters, "iterations", self.iterations)
        self.iterations = self.read_parameter(
            parameters, "astec_NIterations", self.iterations
        )

        self.delta_voxel = self.read_parameter(
            parameters, "delta_voxel", self.delta_voxel
        )
        self.delta_voxel = self.read_parameter(
            parameters, "astec_DeltaVoxels", self.delta_voxel
        )
        ...
        self.processors = self.read_parameter(
            parameters, "astec_nb_proc", self.processors
        )

        self.mimic_historical_astec = self.read_parameter(
            parameters, "mimic_historical_astec", self.mimic_historical_astec
        )
...
class AstecParameters(mars.WatershedParameters, MorphoSnakeParameters):
    ...
    def __init__(self, prefix="astec_"):
        ...
        self.morphosnake_reconstruction = reconstruction.ReconstructionParameters(
            prefix=[self._prefix, "morphosnake_"], suffix="_morphosnake"
        )
        ...
        doc = "\t Anisotropic erosion of cells t\n"
        self.doc["anisotropic_erosion_from_previous"] = doc
        self.anisotropic_erosion_from_previous = False
        ...
        doc += "\t Minimal ratio to determine whether the volume change between mother and daughter cell is too large\n"
        self.doc["volume_decrease_cut_off"] = doc
        self.volume_decrease_cut_off = 0.8

        doc = "\t Volume decrease cut-off for the second round of volume check\n"
        doc += "\t Minimal ratio to determine whether the volume change between mother and daughter cell is too large\n"
        self.doc["second_volume_decrease_cut_off"] = doc
        self.second_volume_decrease_cut_off = None
        #
    ...
    def print_parameters(self):
        ...
        self.varprint(
            ...
            self.propagation_strategy,
            self.doc["propagation_strategy"],
        )

        self.varprint(
            "previous_seg_method",
            self.previous_seg_method,
            self.doc["previous_seg_method"],
        )
        self.varprint(
        ...
        self.varprint(
            "anisotropic_erosion_from_previous",
            self.anisotropic_erosion_from_previous,
            self.doc["anisotropic_erosion_from_previous"],
        )
        self.varprint(
        ...
        self.varprint(
            "seed_selection_tau",
            self.seed_selection_tau,
            self.doc["seed_selection_tau"],
        )
        ...
        self.varprint(
            ...
            self.volume_minimal_value,
            self.doc["volume_minimal_value"],
        )
        self.varprint(
            "volume_decrease_cut_off",
            self.volume_decrease_cut_off,
            self.doc["volume_decrease_cut_off"],
        )
        self.varprint(
            "second_volume_decrease_cut_off",
            self.second_volume_decrease_cut_off,
            self.doc["second_volume_decrease_cut_off"],
        )
        ...
        self.varprint(
            "lineage_diagnosis", self.lineage_diagnosis, self.doc["lineage_diagnosis"]
        )
        print("")
    ...
    def write_parameters_in_file(self, logfile):
        ...
        self.varwrite(
            logfile, "minimum_step", self.minimum_step, self.doc["minimum_step"]
        )
        ...
        self.varwrite(
            logfile,
            "membrane_stringency_factor",
            self.stringency_factor,
            self.doc["stringency_factor"],
        )

        self.varwrite(
            logfile,
            "membrane_ground_truth_time_frame",
            self.gt_time_frame,
            self.doc["gt_time_frame"],
        )
        ...
        self.varwrite(
            logfile,
            "lineage_diagnosis",
            self.lineage_diagnosis,
            self.doc["lineage_diagnosis"],
        )
    ...
    def update_from_parameters(self, parameters):
        ...
        self.propagation_strategy = self.read_parameter(
            parameters, "propagation_strategy", self.propagation_strategy
        )

        #
        #
        #
        self.previous_seg_method = self.read_parameter(
            parameters, "previous_seg_method", self.previous_seg_method
        )
        self.previous_seg_erosion_cell_iterations = self.read_parameter(
            parameters,
            "previous_seg_erosion_cell_iterations",
            self.previous_seg_erosion_cell_iterations,
        )
        self.anisotropic_erosion_from_previous = self.read_parameter(
            parameters,
            "anisotropic_erosion_from_previous",
            self.anisotropic_erosion_from_previous,
        )
        self.previous_seg_erosion_background_iterations = self.read_parameter(
            parameters,
            "previous_seg_erosion_background_iterations",
            self.previous_seg_erosion_background_iterations,
        )
        self.previous_seg_erosion_cell_min_size = self.read_parameter(
            parameters,
            "previous_seg_erosion_cell_min_size",
            self.previous_seg_erosion_cell_min_size,
        )
        ...
        #
        self.background_seed_from_hmin = self.read_parameter(
            parameters, "background_seed_from_hmin", self.background_seed_from_hmin
        )
        self.background_seed_from_previous = self.read_parameter(
            parameters,
            "background_seed_from_previous",
            self.background_seed_from_previous,
        )
        ...
        self.seed_reconstruction.intensity_sigma = self.read_parameter(
            parameters, "seed_intensity_sigma", self.seed_reconstruction.intensity_sigma
        )

        self.seed_reconstruction.intensity_enhancement = self.read_parameter(
            parameters,
            "seed_intensity_enhancement",
            self.seed_reconstruction.intensity_enhancement,
        )

        self.seed_reconstruction.outer_contour_enhancement = self.read_parameter(
            parameters,
            "seed_outer_contour_enhancement",
            self.seed_reconstruction.outer_contour_enhancement,
        )
        ###

        self.seed_selection_tau = self.read_parameter(
            parameters, "seed_selection_tau", self.seed_selection_tau
        )
        self.seed_selection_tau = self.read_parameter(
            parameters, "Thau", self.seed_selection_tau
        )
        # added minimum step
        self.minimum_step = self.read_parameter(
            parameters, "minimum_step", self.minimum_step
        )
        self.minimum_step = self.read_parameter(
            parameters, "Minimum_step", self.minimum_step
        )

        self.minimum_volume_unseeded_cell = self.read_parameter(
            parameters,
            "minimum_volume_unseeded_cell",
            self.minimum_volume_unseeded_cell,
        )

        self.blastocyst_cavity_detection = self.read_parameter(
            parameters, "blastocyst_cavity_detection", self.blastocyst_cavity_detection
        )
        self.cavity_vol_cut_off = self.read_parameter(
            parameters, "cavity_vol_cut_off", self.cavity_vol_cut_off
        )

        self.volume_ratio_tolerance = self.read_parameter(
            parameters, "volume_ratio_tolerance", self.volume_ratio_tolerance
        )
        ...
        self.volume_ratio_tolerance = self.read_parameter(
            parameters, "VolumeRatioSmaller", self.volume_ratio_tolerance
        )

        self.volume_ratio_threshold = self.read_parameter(
            parameters, "VolumeRatioBigger", self.volume_ratio_threshold
        )
        self.volume_minimal_value = self.read_parameter(
            parameters, "volume_minimal_value", self.volume_minimal_value
        )
        self.volume_minimal_value = self.read_parameter(
            parameters, "MinVolume", self.volume_minimal_value
        )
        # added volume_decrease_cut_off
        self.volume_decrease_cut_off = self.read_parameter(
            parameters, "volume_decrease_cut_off", self.volume_decrease_cut_off
        )
        self.second_volume_decrease_cut_off = self.read_parameter(
            parameters,
            "second_volume_decrease_cut_off",
            self.second_volume_decrease_cut_off,
        )
        ...
        self.membrane_sanity_check = self.read_parameter(
            parameters, "membrane_sanity_check", self.membrane_sanity_check
        )
        self.stringency_factor = self.read_parameter(
            parameters, "membrane_stringency_factor", self.stringency_factor
        )
        self.gt_time_frame = self.read_parameter(
            parameters, "membrane_ground_truth_time_frame", self.gt_time_frame
        )
        ###

        self.morphosnake_correction = self.read_parameter(
            parameters, "morphosnake_correction", self.morphosnake_correction
        )
        self.outer_correction_radius_opening = self.read_parameter(
            parameters,
            "outer_correction_radius_opening",
            self.outer_correction_radius_opening,
        )
        self.outer_correction_radius_opening = self.read_parameter(
            parameters, "RadiusOpening", self.outer_correction_radius_opening
        )

        self.lineage_diagnosis = self.read_parameter(
            parameters, "lineage_diagnosis", self.lineage_diagnosis
        )
...
def _erode_cell(args):
    """
    Function that erodes cells from previous time points to make them seed points for initial watershed
    args:
        args (tuple): Collection of input arguments = (tmp, iterations, bb, i, voxelsize, anisotropic_erosion_from_previous)
                    tmp (ndarray): Bounding boxed image of cell
                    iterations (int): How many iterations should be used for the erosion
                    bb (slide object): Bounding box for the eroded cell
                    i (int): label of the eroded cell
                    voxel_size (tuple): voxel size of the image
                    anisotropic_erosion_from_previous (bool): Should the erosion use a structure element that is adapted to the anisotropic voxel size or not
    return:
        eroded (ndarray): Bounding boxed image with eroded cell
        i (int): label of the eroded cell
        bb (slide object): Bounding box for the eroded cell
    """
    #
    ...
    tmp, iterations, bb, i, voxelsize, anisotropic_erosion_from_previous = args
    nb_iter = iterations
    # here we make sure that seeds of which we skip the erosion still make it into the seed image
    if nb_iter == 0:
        eroded = tmp
    else:
        if anisotropic_erosion_from_previous == True and len(set(voxelsize)) > 1:
...
def build_seeds_from_previous_segmentation(
    label_image, output_image, parameters, nprocessors=26
):
    """
        ...
        if size_cell > parameters.previous_seg_erosion_cell_min_size:
            if i == 1:
                mapping.append(
                    (
                        tmp,
                        parameters.previous_seg_erosion_background_iterations,
                        bboxes[i - 1],
                        i,
                        voxelsize,
                        parameters.anisotropic_erosion_from_previous,
                    )
                )
            else:
                mapping.append(
                    (
                        tmp,
                        parameters.previous_seg_erosion_cell_iterations,
                        bboxes[i - 1],
                        i,
                        voxelsize,
                        parameters.anisotropic_erosion_from_previous,
            ...
            if i != 1:
                ...
                monitoring.to_log_and_console(
                    ...
                    + str(size_cell)
                    + ") <= "
                    + str(parameters.previous_seg_erosion_cell_min_size),
                    2,
                )
...
# new function:
def calculate_h_by_measuring_h_range_for_two_seeds_in_all_cells(
    raw_intensity_image,
    ...
    args:
        ...
        parameters: ASTEC class that holds parameters input by user through parameter file
    returns:
    ...
    vol_0, vol_1 = nd.sum(
        np.ones_like(segmentation_image), segmentation_image, index=[0, 1]
    )
    if vol_1 > vol_0:
    ...
    abs_tau = int(
        np.round((parameters.seed_selection_tau / 100) * dr)
    )  # tau is a percentage >> divide by 100
    results_dict = (
        {}
    )  # final result of selected value and wanted number of seeds - label: [h_value, n_seeds]
    correspondences = {}
        ...
        if box is not None:
            ...
            if n_seeds == 1:
                ...
                if len(labels_in_cell) != 2:
                    monitoring.to_log_and_console(
                        f"Something is wrong with cell {label}, it was called dividing but the current time point does not have 2 seeds with the predicted parameters."
                    )
                ...
                full_seed_image[box] = np.where(
                    labels > 0, labels, full_seed_image[box]
                )

            else:  # in case there are no seeds, we retrieve the seed from previous and assume the cell has not divided
                monitoring.to_log_and_console(
                    "       Processing unseeded cell: retrieving seed from previous for cell "
                    + str(label),
                    2,
                )
...
def find_appropriate_number_of_seeds(h_n_seeds_dictionary, abs_tau):
    ...
    if h_diff > abs_tau:
        ...
        if len(one_seed_list) > 0:
            ...
            h_n_seeds_dictionary_no_zeros = {
                key: value for key, value in h_n_seeds_dictionary.items() if value > 0
            }
            if len(h_n_seeds_dictionary_no_zeros) > 0:
                h_value = min(
                    h_n_seeds_dictionary_no_zeros, key=h_n_seeds_dictionary_no_zeros.get
                )
            else:
...
def _compute_volumes(im):
    ...
    if type(im) is str:
        ...
        monitoring.to_log_and_console(
            str(proc) + ": unhandled type for 'im': " + str(type(im)) + "'"
        )
        return
...
def _update_volume_properties(
    lineage_tree_information, segmented_image, current_time, experiment
):
...
def _update_lineage_properties(
    lineage_tree_information, correspondences, previous_time, current_time, experiment
):
    time_digits = experiment.get_time_digits_for_cell_id()
...
def cavity_correction(
    ...
    returns:
        ...
        correspondences (str): Same as above, with corrected cavity (removed from daughter cells)
    ...
    cavity_labels = [
        ...
        if v >= (parameters.cavity_vol_cut_off * median_volume)
    ]
    if cavity_labels:
        corr_rev = {
            value: key
            for key, value_list in correspondences.items()
        ...
        for label in cavity_labels:
            if label > 1:
                ...
                monitoring.to_log_and_console(
                    f"    .. detected cavity: changing cell {label} to background", 1
                )
                ...
                if label in set(correspondences.keys()):
                    correspondences[
                        ...
                        []
                    )  # we need to find a way to allow apoptosis or the dissapearing of the "cavity cell"
                else:
...
def new_membrane_sanity_check(
    ...
    args:
        ...
        previous_segmentation (str): path to the image that the segmentation propagatio starts. All membranes in this images are
                                ...
                                to the labels of the current time point. If cells divided, two labels will be mapped to t-1.
        selected_seeds (str): path to seeds image
        dataframe_path (str): path to csv file that saves the information about membranes from previous time points
        experiment: ASTEC class that holds experiment variables input by user through parameter file
        parameters: ASTEC class that holds parameters input by user through parameter file
        correspondences (dict): Dictionary that maps the labels of the segmentation of t-1
    ...
    new_membrane_ids = [
        value for key, value in mapper.items() if key in newly_div_cell_pairs
    ]

    # calculate volume_ratios for all cells, subset for cells that have divided
    (
        volume_ratios_all_current,
        volumes_all_current,
    ) = membranes.volume_ratio_after_closing(interfaces, mapper, voxelsize)
    volume_ratios_new = {
        key: value
        for key, value in volume_ratios_all_current.items()
        if key in new_membrane_ids
    }
    ...
    if not os.path.isfile(dataframe_path):
        ...
        gt_true_membranes_volumes, gt_volumes = membranes.volume_ratio_after_closing(
            gt_interfaces, gt_mapper, voxelsize
        )
        ...
        add_vol_ratio_df = pd.DataFrame.from_dict(
            gt_true_membranes_volumes, orient="index", columns=["mem_volume_ratio"]
        )
        gt_volumes_df = gt_volumes_df.join(add_vol_ratio_df, on="mem_id")
        add_vol_df = pd.DataFrame.from_dict(
            gt_volumes, orient="index", columns=["volume"]
        )
    ...
    former_uncertain_pairs = list(
        gt_volumes_df.loc[
            (gt_volumes_df["membrane_classification"] == "uncertain")
            & (gt_volumes_df["time_point"] == current_time - 1)
        ]["cells"]
    )

    previous_cell_pairs = set(
        gt_volumes_df.loc[gt_volumes_df["time_point"] == current_time - 1]["cells"]
    )
    max_lineage_id = gt_volumes_df["mem_lineage_id"].astype(int).max()

    append_df = pd.DataFrame(
        index=list(range(0, len(mem_id_add))),
        columns=[
            "mem_id",
            "cells",
            "time_point",
    ...
    for mem_id in mem_id_add:
        ...
        append_df["mem_volume_ratio"][index] = [
            v for k, v in volume_ratios_all_current.items() if k == mem_id
        ][0]
        append_df["volume"][index] = [
            v for k, v in volumes_all_current.items() if k == mem_id
        ][0]

        # convert to id's from the previous time point
        pair_in_prev_ids = membranes.translate_cell_pair_to_previous(
            cell_pair, corr_rev
        )
        ...
        if pair_in_prev_ids in previous_cell_pairs:
            ...
            mem_lineage = gt_volumes_df.loc[
                (gt_volumes_df["time_point"] == current_time - 1)
                & (gt_volumes_df["cells"] == pair_in_prev_ids)
            ]["mem_lineage_id"].values[0]
    ...
    if (not parameters.gt_time_frame == None) and (
        experiment.first_time_point < lower_time_limit
    ):
        considered_gt_membranes = gt_volumes_df.loc[
            gt_volumes_df["time_point"] > lower_time_limit
        ]
    ...
    considered_gt_membranes_certain = considered_gt_membranes.loc[
    ...
    uncertain_membrane_ids = list(
        gt_volumes_df.loc[
            (gt_volumes_df["membrane_classification"] == "uncertain")
            & (gt_volumes_df["time_point"] == current_time)
        ]["mem_id"]
    )

    merged_segmentation_name = common.add_suffix(
        segmentation_image,
        "_after_sanity_check_t" + str("{:03d}".format(current_time)),
        new_dirname=experiment.astec_dir.get_tmp_directory(),
        new_extension=experiment.result_image_suffix,
    )

    merged_seeds_name = common.add_suffix(
        selected_seeds,
        "_after_sanity_check_t" + str("{:03d}".format(current_time)),
        new_dirname=experiment.astec_dir.get_tmp_directory(),
        new_extension=experiment.result_image_suffix,
    ...
    if (len(newly_div_cell_pairs) == 0) & (len(uncertain_membrane_ids) == 0):
        monitoring.to_log_and_console(
            "      .. found no new or uncertain cell membranes: not running membrane sanity check",
            2,
        )
        ...
        monitoring.to_log_and_console(
            "      .. found dividing cells or uncertain membranes: running membrane sanity check",
            2,
        )
        volume_ratios_uncertain = {
            key: value
            for key, value in volume_ratios_all_current.items()
            if key in uncertain_membrane_ids
        }

        # define which membranes are false and merge pairs of cells that are connected through that membrane
        new_for_fusion = [
            mem_id for mem_id, volume in volume_ratios_new.items() if volume > cut_off
        ]
        uncertain_for_fusion = [
            mem_id
            for mem_id, volume in volume_ratios_uncertain.items()
            if volume > cut_off
        ]

        passed_new_membranes = [
            mem_id for mem_id, volume in volume_ratios_new.items() if volume <= cut_off
        ]
        passed_uncertain_membranes = [
            mem_id
            for mem_id, volume in volume_ratios_uncertain.items()
            if volume <= cut_off
        ]
        ...
        new_false_pairs = [
            key for key, value in mapper.items() if value in new_for_fusion
        ]
        uncertain_false_pairs = [
            key for key, value in mapper.items() if value in uncertain_for_fusion
        ]
        false_pairs_list = new_false_pairs + uncertain_false_pairs
        if len(false_pairs_list) > 0:
            monitoring.to_log_and_console(
                "      .. fusing cell pairs:" + f"{false_pairs_list}", 2
            )
            ...
            (
                ...
                updated_seeds,
                changed_cells,
            ) = membranes.merge_labels_with_false_membranes(
                false_pairs_list, curr_seg, updated_seeds, correspondences
            )
            # output will first be saved in tmp and copied to main in astec_process as final result of membrane sanity check
            imsave(
                merged_segmentation_name,
                SpatialImage(merged_segmentation, voxelsize=voxelsize).astype(
                    np.uint16
                ),
            )
            imsave(
                merged_seeds_name,
                SpatialImage(updated_seeds, voxelsize=voxelsize).astype(np.uint16),
            )
            correspondences = membranes.update_correspondences_dictionary(
                correspondences, changed_cells
            )
        else:
            monitoring.to_log_and_console(
                "      .. did not find false membranes, not fusing any cell pairs", 2
            )
            imsave(
                merged_segmentation_name,
                SpatialImage(curr_seg, voxelsize=voxelsize).astype(np.uint16),
            )
            imsave(
                merged_seeds_name,
                SpatialImage(updated_seeds, voxelsize=voxelsize).astype(np.uint16),
        ...
        for mem_id in uncertain_for_fusion:
            index = gt_volumes_df.loc[
                (gt_volumes_df["mem_id"] == mem_id)
                & (gt_volumes_df["time_point"] == current_time)
            ].index[0]
...
def _volume_diagnosis(
    prev_volumes, curr_volumes, correspondences, volume_decrease_cut_off
):
    """
    ...
    for mother_c, daughters_c in correspondences.items():
        ...
        if mother_c in prev_volumes is False:
            ...
            # compare ratio to cut-off
                ...
                vol_ratio = 1 / (
                    prev_volumes[mother_c]
                    / (curr_volumes[daughters_c[0]] + curr_volumes[daughters_c[1]])
                )
                if vol_ratio < volume_decrease_cut_off:
                    cells_with_volume_loss[mother_c] = daughters_c
            else:
                monitoring.to_log_and_console(
                    ...
                    + ": cell "
                    + str(mother_c)
                    + " has an anomalie in the number of daughter cells (none, or more than 2)",
                    2,
                )
...
def _volume_decrease_correction(
    astec_name,
    intensity_seed_image,
    previous_segmentation,
    ...
    if (
        -parameters.volume_ratio_tolerance
        <= volume_ratio
        <= parameters.volume_ratio_tolerance
    ):
        pass
        ...
        if volume_ratio < 0:
            monitoring.to_log_and_console(
                "      .. warning: embryo volume has strongly diminished", 2
            )
        else:
            monitoring.to_log_and_console(
                "      .. warning: embryo volume has strongly increased", 2
            )

    cells_with_volume_loss = _volume_diagnosis(
        prev_volumes, curr_volumes, correspondences, parameters.volume_decrease_cut_off
    )

    if len(cells_with_volume_loss) > 0:
        monitoring.to_log_and_console(
            "        process cell(s) with unusually large decrease in volume: "
            + str(cells_with_volume_loss),
            2,
        )
    else:
    ...
    b_boxes = membranes.calculate_incremented_bounding_boxes(
        segmentation_from_previous, increment=3
    )
    ...
    for cell in cells_with_volume_loss.keys():
        ...
        if len(siblings) == 1:
            ...
            while seeds_to_assign:
                ...
                if (
                    (round == 0)
                    and (len(unique_to_a) == 0)
                    and (len(unique_to_b) == 0)
                    and (len(touching_both) == 0)
                ):
                    monitoring.to_log_and_console(
                        f"       ... found no extra seeds for divided cell {cell}, seeding remains unchanged despite volume loss",
                        2,
                    )
                    break
                ...
                if skip_a == False:
                    for unique_seed in unique_to_a:
                        ...
                        mask_cell_a = np.where(
                            tmp_watershed == unique_seed, True, mask_cell_a
                        )
                ...
                if skip_b == False:
                    for unique_seed in unique_to_b:
                        ...
                        mask_cell_b = np.where(
                            tmp_watershed == unique_seed, True, mask_cell_b
                        )
                        # remove from list of all labels
                        seeds_to_assign.remove(unique_seed)
                ...
                if len(touching_both) > 0 and skip_a == False and skip_b == False:
                    for double_seed in touching_both:
                        if double_seed in seeds_to_assign:
                            ...
                            vol_with_a = np.sum(
                                np.logical_and(
                                    tmp_watershed == double_seed, dil_mask_cell_a
                                )
                            )
                            vol_with_b = np.sum(
                                np.logical_and(
                                    tmp_watershed == double_seed, dil_mask_cell_b
                                )
                            )
                            if vol_with_a > vol_with_b:
                                full_seed_image[box][labels == double_seed] = siblings[
                                    0
                                ]
    ...
    if change_in_seeds == 0:
        monitoring.to_log_and_console(
            "    .. no changes in seeds, do not recompute segmentation", 2
        )
        return segmentation_from_selection, selected_seeds, cells_with_volume_loss
        ...
        monitoring.to_log_and_console(
            "    .. "
            + str(change_in_seeds)
            + " changes in seeds, recompute segmentation",
            2,
        )

        corr_selected_seeds = common.add_suffix(
            astec_name,
            "_seeds_from_corrected_selection",
            new_dirname=experiment.astec_dir.get_tmp_directory(),
        ...
        imsave(
            corr_selected_seeds,
            SpatialImage(full_seed_image, voxelsize=voxelsize).astype(np.uint16),
        )
        del full_seed_image

        segmentation_from_corr_selection = common.add_suffix(
            astec_name,
            "_watershed_from_corrected_selection",
            new_dirname=experiment.astec_dir.get_tmp_directory(),
            new_extension=experiment.default_image_suffix,
        )
        mars.watershed(
            corr_selected_seeds,
            membrane_image,
            segmentation_from_corr_selection,
            parameters,
        )
        return (
            segmentation_from_corr_selection,
...
def _volume_decrease_correction_check(
    astec_name,
    membrane_image,
    previous_segmentation,
    cells_with_volume_loss,
    ...
    (either above second_volume_decrease_cut_off, or above the threshold used in the previous round).
    ...
    prev_volumes = _compute_volumes(previous_segmentation)
    curr_volumes = _compute_volumes(current_segmentation)
    if parameters.second_volume_decrease_cut_off == None:
        cut_off = parameters.volume_decrease_cut_off
    else:
        cut_off = parameters.second_volume_decrease_cut_off

    cells_with_volume_loss = _volume_diagnosis(
        prev_volumes, curr_volumes, correspondences, cut_off
    )
    ...
    for cell in cells_with_volume_loss:
        ...
        if len(siblings) == 1:
            ...
            print(f"cell {cell} too small after correction, using seed from previous")
    ...
    imsave(
        selected_seeds,
        SpatialImage(current_seed_image, voxelsize=voxelsize).astype(np.uint16),
    )
    del current_seed_image

    segmentation_from_corr_selection = common.add_suffix(
        astec_name,
        "_watershed_from_double_corrected_selection",
        new_dirname=experiment.astec_dir.get_tmp_directory(),
        new_extension=experiment.default_image_suffix,
    )
    mars.watershed(
        selected_seeds, membrane_image, segmentation_from_corr_selection, parameters
    )
...
def _morphosnakes(parameters_for_parallelism):
    ...
    initialization = nd.binary_dilation(
        subsegmentation == mother_c, iterations=astec_parameters.dilation_iterations
    )
    if write_images:
    ...
    if energy.lower() == "gradient":
        ...
        monitoring.to_log_and_console(
            str(proc) + ": unknown energy function, switch to image-based"
        )
        energy = "image"
    ...
    if energy.lower() == "gradient":
        ...
        energy = 1.0 / np.sqrt(1 + 100 * energy)
    else:
        # energy.lower() == 'image':
        ...
        energy = 1.0 / np.sqrt(
            1
            + 100
            * (subimage.astype(float) - subimage_min)
            / (subimage_max - subimage_min)
        )
        del subimage
    ...
    macwe = morphsnakes.MorphGAC(
        energy,
        smoothing=astec_parameters.smoothing,
        threshold=1,
    ...
    for i in range(0, astec_parameters.iterations, step):
        ...
        if (
            np.sum(before != macwe.levelset) < astec_parameters.delta_voxel
...
def _historical_morphosnakes(parameters_for_parallelism):
    ...
    #
    subsegmentation = imread(subsegmentation_name)
    initialization = nd.binary_erosion(
        subsegmentation != mother_c,
        iterations=astec_parameters.dilation_iterations,
        border_value=1,
    )
    if write_images:
        initialization_name = common.add_suffix(subsegmentation_name, "_initialization")
...
def _outer_volume_decrease_correction(
    astec_name,
    previous_segmentation,
    ...
    cells_with_volume_loss = _volume_diagnosis(
        prev_volumes, curr_volumes, correspondences, parameters
    )
    ...
    if len(cells_with_volume_loss) > 0:
        monitoring.to_log_and_console(
            "        process cell(s) with large decrease of volume (morphosnake)", 2
        )
    else:
        del prev_seg
        del curr_seg
    ...
    if len(exterior_correction) == 0:
        ...
        del curr_seg
        monitoring.to_log_and_console(
            "        .. no cells with large background part", 2
        )
        return segmentation_from_selection, correspondences, []
    ...
    #
    monitoring.to_log_and_console(
        "        .. (mother) cell(s) to be corrected (morphosnake step): "
        + str(exterior_correction),
        2,
    )
    ...
    for mother_c in exterior_correction:
        ...
        #
        bb = _slices_dilation(
            bounding_boxes[mother_c - 1],
            maximum=prev_seg.shape,
            iterations=parameters.dilation_iterations + 5,
        )
        #
        # subimages
        #
        subsegmentation = os.path.join(
            experiment.astec_dir.get_tmp_directory(),
            "cellsegment_" + str(mother_c) + "." + experiment.default_image_suffix,
        )
        subimage = os.path.join(
            experiment.astec_dir.get_tmp_directory(),
            "cellimage_" + str(mother_c) + "." + experiment.default_image_suffix,
        )
    ...
    #
    monitoring.to_log_and_console(
        "        .. cell(s) that have been corrected (morphosnake step): "
        + str(effective_exterior_correction),
        2,
    )

    segmentation_after_morphosnakes = common.add_suffix(
        astec_name,
        "_morphosnakes",
        new_dirname=experiment.astec_dir.get_tmp_directory(),
        new_extension=experiment.default_image_suffix,
    ...
    #
    return (
        segmentation_after_morphosnakes,
        correspondences,
        effective_exterior_correction,
    )


def _outer_morphosnake_correction(
    astec_name,
    input_segmentation,
    ...
    #
    #
    opening_name = common.add_suffix(
        astec_name,
        "_morphosnakes_opening",
        new_dirname=experiment.astec_dir.get_tmp_directory(),
        new_extension=experiment.default_image_suffix,
    )
    options = "-opening -R " + str(parameters.outer_correction_radius_opening)
    cpp_wrapping.mathematical_morphology(
        input_segmentation, opening_name, other_options=options, monitoring=monitoring
    )
    ...
    for daughter_c in effective_exterior_correction:
        segmentation[
            ((segmentation == daughter_c) & (reference == 1) & (opening == 1))
        ] = 1
...
def astec_process(
    previous_time, current_time, lineage_tree_information, experiment, parameters
):
    """
    ...
    if astec_image is not None:
        if monitoring.forceResultsToBeBuilt is False:
            ...
            monitoring.to_log_and_console(
                "    astec image already existing, but forced", 2
            )

    astec_image = os.path.join(
        astec_dir, astec_name + "." + experiment.result_image_suffix
    )
    ...
    if (
        ...
        deformation = reconstruction.get_deformation_from_current_to_previous(
            current_time, experiment, parameters.membrane_reconstruction, previous_time
        )
    ...
    if parameters.previous_seg_method.lower() == "deform_then_erode":
        monitoring.to_log_and_console(
            "      transform previous segmentation then erode it", 2
        )

        if (
            not os.path.isfile(deformed_seeds)
            or monitoring.forceResultsToBeBuilt is True
        ):
            build_seeds_from_previous_segmentation(
                deformed_segmentation, deformed_seeds, parameters
            )

    elif parameters.previous_seg_method.lower() == "erode_then_deform":
        monitoring.to_log_and_console(
            "      erode previous segmentation then transform it", 2
        )
        eroded_seeds = common.add_suffix(
            astec_name,
            "_eroded_seeds_from_previous",
            new_dirname=experiment.astec_dir.get_tmp_directory(),
            new_extension=experiment.default_image_suffix,
        )
        if not os.path.isfile(eroded_seeds) or monitoring.forceResultsToBeBuilt is True:
            build_seeds_from_previous_segmentation(
                previous_segmentation, eroded_seeds, parameters
            )
        ...
        monitoring.to_log_and_console(
            ...
            + proc
            + ": unknown method '"
            + str(parameters.previous_seg_method)
    ...
    if parameters.propagation_strategy == "seeds_from_previous_segmentation":
        ...
        segmentation_from_previous = common.add_suffix(
            astec_name,
            "_watershed_from_previous",
            new_dirname=experiment.astec_dir.get_tmp_directory(),
            new_extension=experiment.default_image_suffix,
        )
    ...
    #
    if (
        not os.path.isfile(segmentation_from_previous)
        or monitoring.forceResultsToBeBuilt is True
    ):
        mars.watershed(
            deformed_seeds, membrane_image, segmentation_from_previous, parameters
        )
    ...
    if parameters.propagation_strategy == "seeds_from_previous_segmentation":
        ...
        #
        lineage_tree_information = _update_volume_properties(
            lineage_tree_information, astec_image, current_time, experiment
        )
        ...
        lineage_tree_information = _update_lineage_properties(
    ...
    if parameters.seed_reconstruction.is_equal(parameters.membrane_reconstruction):
        ...
        image_for_seed = reconstruction.build_reconstructed_image(
            current_time,
            experiment,
    ...
    if image_for_seed is None:
        monitoring.to_log_and_console(
            "    .. "
            + proc
            + " no fused image was found for time "
            + str(current_time),
            2,
        )
        return None
    ...
    selected_seeds = common.add_suffix(
        astec_name,
        "_seeds_from_selection",
        new_dirname=experiment.astec_dir.get_tmp_directory(),
        new_extension=experiment.default_image_suffix,
    )
    ...
    if not os.path.isfile(selected_seeds) or monitoring.forceResultsToBeBuilt is True:
        (
            results_division,
            correspondences,
        ) = calculate_h_by_measuring_h_range_for_two_seeds_in_all_cells(
            raw_intensity_image,
            image_for_seed,
            segmentation_from_previous,
            path_to_previous_seeds,
        ...
        results_division_path = os.path.join(
            str(experiment.astec_dir.get_tmp_directory()), "results_division.pkl"
        )
        ...
        correspondences_path = os.path.join(
            str(experiment.astec_dir.get_tmp_directory()), "correspondences.pkl"
        )
        ...
        dividing_cells = [
            mother
            for mother, result_list in results_division.items()
            if result_list[1] >= 2
        ]
        if len(dividing_cells) > 0:
            ...
            monitoring.to_log_and_console(
                "    .. no dividing cell between time point "
                + str(previous_time)
                + " and "
                + str(current_time),
                2,
        ...
        monitoring.to_log_and_console(
            "    .. selected seeds image already exists, proceed to watershed ", 2
        )
        results_division_path = (
            str(experiment.astec_dir.get_tmp_directory()) + "/results_division.pkl"
        )
    ...
    if parameters.propagation_strategy == "seeds_selection_without_correction":
        ...
        segmentation_from_selection = common.add_suffix(
            astec_name,
            "_watershed_from_selection",
            new_dirname=experiment.astec_dir.get_tmp_directory(),
            new_extension=experiment.default_image_suffix,
        )

    if (
        not os.path.isfile(segmentation_from_selection)
        or monitoring.forceResultsToBeBuilt is True
    ):
        mars.watershed(
            selected_seeds, membrane_image, segmentation_from_selection, parameters
        )
    ...
    if parameters.blastocyst_cavity_detection == True:
        output_segmentation = common.add_suffix(
            astec_name,
            "_cavity_corrected_segmentation",
            new_dirname=experiment.astec_dir.get_tmp_directory(),
            new_extension=experiment.default_image_suffix,
        )
        monitoring.to_log_and_console("         ...running cavity detection")
        cavity_corrected_segmentation, correspondences = cavity_correction(
            input_segmentation,
            selected_seeds,
            output_segmentation,
            correspondences,
    ...
    segmentation_from_selection, selected_seeds = _volume_decrease_correction_check(
        astec_name,
        image_for_seed,
        previous_segmentation,
        cells_with_volume_loss,
    ...
    if parameters.morphosnake_correction is True:
        monitoring.to_log_and_console(
            "    outer volume decrease correction (morphosnakes)", 2
        )
        #
        ...
        # segmentation_from_previous
        #
        if parameters.morphosnake_reconstruction.is_equal(
            parameters.membrane_reconstruction
        ):
            monitoring.to_log_and_console(
                "    .. morphosnake image is identical to membrane image", 2
            )
            image_for_morphosnake = membrane_image
        elif parameters.morphosnake_reconstruction.is_equal(
            parameters.seed_reconstruction
        ):
            monitoring.to_log_and_console(
                "    .. morphosnake image is identical to seed image", 2
            )
            image_for_morphosnake = image_for_seed
        else:
            image_for_morphosnake = reconstruction.build_reconstructed_image(
                current_time,
                experiment,
        ...
        output = _outer_volume_decrease_correction(
            astec_name,
            previous_segmentation,
            deformed_segmentation,
            input_segmentation,
            image_for_morphosnake,
            correspondences,
            experiment,
            parameters,
        )
    ...
    if len(effective_exterior_correction) > 0:
        ...
        output_segmentation = common.add_suffix(
            astec_name,
            "_morphosnakes_correction",
            new_dirname=experiment.astec_dir.get_tmp_directory(),
            new_extension=experiment.default_image_suffix,
        )
        _outer_morphosnake_correction(
            astec_name,
            input_segmentation,
            reference_segmentation,
            output_segmentation,
...
def _get_last_time_from_lineage(
    lineage_tree_information,
    first_time_point,
    delta_time_point=1,
    ...
    if (
        len(lineage_tree_information) > 0
        and ioproperties.keydictionary["lineage"]["output_key"]
        in lineage_tree_information
    ):
        ...
        #
        for c in lineage_tree_information[
            ioproperties.keydictionary["lineage"]["output_key"]
        ]:
            t = int(c) / div
...
def _get_last_time_from_images(experiment, first_time_point, delta_time_point=1):
    ...
    while True:
        segmentation_file = experiment.get_segmentation_image(
            current_time, verbose=False
        )
...
def _clean_lineage(
    lineage_tree_information, first_time_point, time_digits_for_cell_id=4
):
    #
        ...
        if key == ioproperties.keydictionary["lineage"]["output_key"]:
            ...
            for k in tmp:
                if int(k) > first_time_point * mul:
                    del lineage_tree_information[
                        ioproperties.keydictionary["lineage"]["output_key"]
                    ][k]
        elif key == ioproperties.keydictionary["volume"]["output_key"]:
            tmp = copy.deepcopy(
                lineage_tree_information[
                    ioproperties.keydictionary["volume"]["output_key"]
                ]
            )
            for k in tmp:
            ...
            monitoring.to_log_and_console(
                str(proc) + ": unhandled key '" + str(key) + "'"
            )
    return
...
def _clean_images(experiment, first_time_point, last_time_point, delta_time_point=1):
    ...
    for current_time in range(
        first_time_point, last_time_point + 1, experiment.delta_time_point
    ):
        segmentation_file = experiment.get_segmentation_image(
            current_time, verbose=False
        )
...
def _fill_volumes(lineage_tree_information, first_time_point, experiment):
    ...
    if lineage_tree_information != {}:
        if (
            ...
            tmp = lineage_tree_information[
                ioproperties.keydictionary["volume"]["output_key"]
            ]
            for k in tmp:
                if k / mul == first_time_point:
                    monitoring.to_log_and_console(
                        "    .. cell volumes found for time #" + str(first_time_point),
                        1,
                    )
                    return lineage_tree_information
    #
    ...
    if first_segmentation is None:
        monitoring.to_log_and_console(
            ".. "
            + proc
            + ": no segmentation image found for time '"
            + str(first_time_point)
            + "'",
            1,
        )
        ...
        sys.exit(1)
    monitoring.to_log_and_console(
        "    .. computes cell volumes for time #" + str(first_time_point), 1
    )
    return _update_volume_properties(
        lineage_tree_information, first_segmentation, first_time_point, experiment
    )
...
def astec_control(experiment, parameters):
    ...
    if lineage_tree_file is not None and os.path.isfile(
        os.path.join(segmentation_dir, lineage_tree_file)
    ):
    ...
    last_time_lineage = _get_last_time_from_lineage(
        lineage_tree_information,
        first_time_point,
        delta_time_point=experiment.delta_time_point,
        time_digits_for_cell_id=time_digits_for_cell_id,
    )
    last_time_images = _get_last_time_from_images(
        experiment, first_time_point, delta_time_point=experiment.delta_time_point
    )
    ...
    if restart <= last_time_point:
        monitoring.to_log_and_console(
            ".. " + proc + ": start computation at time #" + str(restart), 1
        )
    else:
    ...
    if restart <= last_time_point:
        monitoring.to_log_and_console(
            "    .. clean lineage and segmentation directory", 1
        )
        _clean_lineage(
            lineage_tree_information,
            restart - delta_time_point,
            time_digits_for_cell_id=time_digits_for_cell_id,
        )
        _clean_images(
            experiment, restart, last_time_point, delta_time_point=delta_time_point
        )
    ...
    #
    if restart <= last_time_point:
        lineage_tree_information = _fill_volumes(
            lineage_tree_information, restart - delta_time_point, experiment
        )
    ...
    #
    for current_time in range(
        restart, last_time_point + 1, experiment.delta_time_point
    ):
        ...
        monitoring.to_log_and_console(
            "... astec processing of time #" + acquisition_time, 1
        )
        ...
        if ret is False:
            monitoring.to_log_and_console(
                "    an error occurs when processing time " + acquisition_time, 1
            )
            return False
        else:
