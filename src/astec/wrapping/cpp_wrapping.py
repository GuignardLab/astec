...
def __find_exec(executable_file):
    ...
    try:
        ...
        try_file = os.path.join(
            os.path.dirname(__file__), "cpp", "vt", "build", "bin", str(executable_file)
        )
...
def _find_exec(executable_file, monitoring=None):
    ...
    if path_to_exec is None:
        _write_error_msg(
            "findExec: can not find executable '" + str(executable_file) + "'",
            monitoring,
        )
...
def _launch_inline_cmd(command_line, monitoring=None):
    ...
    if monitoring is not None and (monitoring.verbose >= 2 or monitoring.debug > 0):
        ...
        with open(monitoring.log_filename, "a") as logfile:
            subprocess.call(
                command_line, shell=True, stdout=logfile, stderr=subprocess.STDOUT
            )
    else:
        subprocess.call(
...
def applyTrsf(
    ...
    if type(interpolation_mode) == str:
        if interpolation_mode.lower() == "linear":
            ...
            if (type(cell_based_sigma) == int and cell_based_sigma > 0) or (
                type(cell_based_sigma) == float and cell_based_sigma > 0.0
            ):
    ...
    if voxel_size is not None:
        if type(voxel_size) == int or type(voxel_size) == float:
            ...
            if len(voxel_size) == 3:
                ...
                _write_error_msg(
                    proc
                    + ": unhandled length for voxel_size '"
                    + str(len(voxel_size))
                    + "'",
                    monitoring,
                )
    ...
    if floating_voxel is not None and voxel_size is not None:
        if type(floating_voxel) == int or type(floating_voxel) == float:
            ...
            if len(floating_voxel) == 3:
                command_line += (
                    " -floating-voxel "
                    + str(floating_voxel[0])
                    + " "
                    + str(floating_voxel[1])
                    + " "
                    + str(floating_voxel[2])
                )
            else:
                _write_error_msg(
                    proc
                    + ": unhandled length for floating image voxel size'"
                    + str(len(floating_voxel))
                    + "'",
                    monitoring,
                )
                _write_error_msg("\t Exiting", monitoring)
            ...
            _write_error_msg(
                proc
                + ": unhandled type for floating image voxel size'"
                + str(type(floating_voxel))
                + "'",
                monitoring,
            )
            _write_error_msg("\t Exiting", monitoring)
    ...
    if dimensions is not None:
        if type(dimensions) == tuple or type(dimensions) == list:
            if len(dimensions) == 3:
                ...
                _write_error_msg(
                    proc
                    + ": unhandled length for dimensions '"
                    + str(len(dimensions))
                    + "'",
                    monitoring,
                )
...
def blockmatching(
    ...
    command_line += (
        " -pyramid-highest-level " + str(py_hl) + " -pyramid-lowest-level " + str(py_ll)
    )
    ...
    if (
        transformation_type.lower() == "vectorfield"
        or transformation_type.lower() == "vectorfield2d"
...
def mc_mask_seeds(
    seed_image, cell_image, seed_result, other_options=None, monitoring=None
):
...
def apply_transformation(
    ...
    applyTrsf(
        the_image,
        res_image,
        the_transformation=the_transformation,
        template_image=template_image,
        res_transformation=res_transformation,
        voxel_size=voxel_size,
        floating_voxel=voxel_size,
    ...
    if interpolation_mode.lower() == "nearest":
        if os.path.isfile(res_image):
            ...
            command_line = (
                path_to_exec + " " + res_image + " " + res_image + " -modify 0 1"
            )
            _launch_inline_cmd(command_line, monitoring=monitoring)
...
def slitline_correction(
    the_image,
    res_image,
    output_corrections=None,
...
def linear_registration(
    ...
    if path_init_trsf is not None:
        command_line += (
            " -init-res-trsf " + path_init_trsf + " -composition-with-initial"
        )
    command_line += " -res-trsf " + path_output_trsf
...
def linear_combination(
    the_weights, the_images, res_image, other_options=None, monitoring=None
):
    """
...
def change_multiple_trsfs(
    ...
    command_line = (
        path_to_exec + " -format " + format_input + " -res-format " + format_output
    )
    command_line += " -first " + str(first) + " -last " + str(last)
    ...
    if resolution is not None:
        if (type(resolution) is tuple or type(resolution) is list) and len(
            ...
            command_line += (
                str(resolution[0]) + " " + str(resolution[1]) + " " + str(resolution[2])
            )
...
def obsolete_gradient_norm(image_input, gradient_output):
    ...
    os.system(
        path_gradient_norm + " " + image_input + " " + gradient_output + " -sigma 1"
    )
...
def watershed(
    path_seeds, path_gradient, path_output, other_options=None, monitoring=None
):
    """
...
def global_intensity_normalization(
    path_input,
    path_output,
    min_percentile=0.01,
    max_percentile=0.99,
...
def cell_intensity_normalization(
    path_input,
    path_segmentation,
    path_output,
    min_percentile=0.01,
...
def membrane_extraction(
    path_input,
    prefix_output="tmp_membrane",
    path_mask=None,
    scale=0.9,
    real_scale=True,
    other_options=None,
...
def anisotropic_histogram(
    path_input_extrema,
    path_output_histogram,
    path_output,
    path_input_mask=None,
    manual=False,
    manual_sigma=7,
    sensitivity=0.98,
    other_options=None,
...
def tensor_voting_membrane(
    path_input,
    prefix_input,
    path_output,
    path_mask=None,
    scale_tensor_voting=3.6,
    sigma_smoothing=0.9,
    real_scale=True,
    sample=0.2,
    random_seed=None,
    ...
    if isnonzero == 0:
        if monitoring is not None:
            monitoring.to_log(
                proc
                + ": '"
                + str(path_input).split(os.path.sep)[-1]
                + "' only contains 0."
            )
    ...
    command_line = (
        path_to_exec + " " + path_input + " -output-eigenvalues " + prefix_input
    )
    if path_mask is not None and os.path.isfile(path_mask):
        command_line += " -mask " + path_mask
    ...
    arithmetic_operation(
        prefix_input + ".imvp3.inr",
        prefix_input + ".imvp1.inr",
        prefix_input + ".tv.inr",
        other_options="-sub",
    )
...
def mathematical_morphology(
    path_input, path_output, other_options=None, monitoring=None
):
    """
...
def obsolete_seuillage(
    path_input,
    path_output="tmp_threshold.inr",
    sb=1,
    sh=None,
    grey=False,
    lazy=True,
...
def obsolete_membrane_renforcement(
    path_input,
    prefix_output="tmp_membrane",
    path_mask=None,
    init=0.9,
...
def _recfilter(
    path_input, path_output="tmp.inr", filter_value=2, rad_min=1, lazy=False
):
    ...
    """
    print(
        "recfilter: WARNING: This function is obsolete. The user should replace its use by function linearfilter."
    )
    os.system(
        path_filters
        + " "
...
def obsolete_watershed(
    ...
    command = path_watershed + " " + path_seeds + " " + path_int + " " + path_output
...
def _reech3d(im_path, output_shape):
    ...
    os.system(
        ...
        + tmp_file
        + " -x "
        + str(int(output_shape[0]))
        + " -y "
        + str(int(output_shape[1]))
        + " -z "
        + str(int(output_shape[2]))
    )
    out = imread(tmp_file)
...
def _reech(path_flo, path_output, voxelsize):
    ...
    """
    os.system(
        path_reech3d + " " + path_flo + " " + path_output + " -linear"
        " -iso " + str(voxelsize)
    )


def obsolete_non_linear_registration(
    image_file_flo,
    image_file_ref,
    affine_image,
    affine_trsf,
...
def obsolete_find_local_minima(
    path_out, path_ref, h_min, mask=None, sigma=0.6, verbose=False
):
...
def outer_detection(im_ref_tmp, radius, seg_ref_tmp):
    ...
    mask = np.uint8(imE)
    bounds = (
        nd.binary_dilation(mask, structure=nd.generate_binary_structure(3, 1)) - mask
    )
...
def obsolete_anisotropicHist(
    path_input="temp_membrane.ext.inr",
    path_output="tmp_membrane.bin.inr",
    path_mask=None,
    manual=False,
    manual_sigma=7,
    sensitivity=0.98,
    ...
    assert os.path.dirname(path_output) == "" or os.path.exists(
    ...
    assert (
        ...
        and os.path.exists(path_theta)
        and os.path.exists(path_phi)
    )
    ...
    if True:
        i = 1
        path_output_theta = (
            ".".join(path_output.rstrip(".gz").split(".")[0:i]) + ".theta.inr"
        )
        path_output_phi = (
            ".".join(path_output.rstrip(".gz").split(".")[0:i]) + ".phi.inr"
        )
        if (not os.path.exists(path_output_theta)) or (
            not os.path.exists(path_output_phi)
        ):
...
def obsolete_TVmembrane(
    path_input="temp_membrane.bin.inr",
    path_output="tmp_TVmembrane.VP31.inr",
    path_mask=None,
    scale=3.6,
    sigma_LF=0.9,
    realScale=True,
    ...
    if obsolete_nonZerosImage(path_input):
        ...
        if not keepAll:
            ...
            files_to_erase.replace(
                " " + path_output.lstrip().rstrip(), ""
            )  # in case path_output would correspond to one of the file names automatically generated by TVmembrane...
        ...
        if verbose:
            print(
                "WARNING : TVmembrane INPUT IMAGE "
                + path_input
                + " SEEMS TO CONTAIN ONLY VOXELS WITH NULL VALUE /!\\"
            )
        path_copy = _find_exec("copy")
...
def directionHistogram(
    path_input, path_output, rayon=None, sigma=None, verbose=0, lazy=True
):
    """
    ...
    commande_shell = (
        path_directionHistogram + " " + path_input + " " + path_output + " " + options
    )
...
def directionHistogramMaxima(
    path_input, file_out=None, maxima=None, frac=None, verbose=False
):
    """
...
def dice(
    path_image, file_out=None, symmetry=None, binary=False, verbose=False, lazy=True
):
    """
    ...
    if symmetry:
        if type(symmetry) == str and os.path.exists(
            symmetry
        ):  # means user gave a file-name of the equation of symmetry plane
            f = open(symmetry)
            n = []
            for line in f:
                li = line.strip()
            ...
            n = symmetry
        options += " -n " + str(n).replace("n:", "").replace(",", "").replace(
            "[", ""
        ).replace("]", "").replace("'", "")
    if binary:
        options += " -bin "
...
def read_dices(file):
    ...
    for k in list(dico.keys()):
        dico[k]["dice_max"] = max(
            dico[k]["dice_left_right"] + dico[k]["dice_right_left"]
        )
...
def symmetryPlane(
    path_input_image,
    path_input_sphere=None,
    normal=None,
    equation_output="tmp_symmetryPlane",
    plane_output=None,
    ...
    if normal:
        options += " -n " + str(normal).replace("n:", "").replace(",", "").replace(
            "[", ""
        ).replace("]", "").replace(
            "'", ""
        )  # normal peut etre le 'string' deja formate ou bien une list ou array contenant les 4 flottants attendus
...
def planeRegistration(
    ...
    path_residuals="",
    path_dices="",
    path_pairs_ref_flo="tmp_planeRegistration_pairs",
    background_ref=1,
    background_flo=1,
    trsf_type="affine",
    estimator="lts",
    ...
    if plane_eq_ref:
        ...
        if os.path.exists(str(plane_eq_ref)):
            ...
            n = plane_eq_ref
        options += " -p-ref " + str(n).replace("n:", "").replace(",", "").replace(
            "[", ""
        ).replace("]", "").replace(
            "'", ""
        )  # normal peut etre le 'string' deja formate ou bien un vecteur contenant les 4 flottants attendus
    if plane_eq_flo:
        if os.path.exists(str(plane_eq_flo)):
            ...
            n = plane_eq_flo
        options += " -p-flo " + str(n).replace("n:", "").replace(",", "").replace(
            "[", ""
        ).replace("]", "").replace(
            "'", ""
        )  # normal peut etre le 'string' deja formate ou bien un vecteur contenant les 4 flottants attendus
...
def fuseLabelsWithLUT(
    image_in, image_out, path_lut, U8=False, lazy=True, verbose=False
):
    """
...
def connexe_with_options(
    path_input,
    path_output="tmp_threshold",
    path_seeds=None,
    sb=None,
    sh=None,
    tcc=None,
    ...
    os.system(path_connexe + " " + path_input + " " + path_output + option)
    if not lazy:
        out = imread(path_output)
        os.system("rm " + path_output)
        return out
...
def obsolete_compose_trsf(
    path_trsf_1, path_trsf_2, path_output="tmp_compose.inr", lazy=True, verbose=False
):
    """
...
def obsolete_Logic(
    image_in, image_ext_or_out, image_out=None, Mode=None, lazy=True, verbose=False
):
    """
...
def obsolete_createImage(
    image_out, image_template, options="", lazy=True, verbose=False
):
    """
...
def erodeLabels(image_in, image_out, r=None, lazy=True, verbose=False):
    ...
    if image_tmp == image_in:
        if image_tmp.count(os.path.sep):
            image_tmp = (
                os.path.sep.join(image_tmp.split(os.path.sep)[:-1])
                + os.path.sep
                + "erodeLabels_tmp.inr"
            )
        else:
...
def obsolete_patchLogic(
    image_patch, image_ext, image_out, origin, Mode=None, verbose=False
):
    """
...
def obsolete_mc_adhocFuse(
    image_fuse,
    image_seg,
    image_out,
    min_percentile=0.01,
    max_percentile=0.99,
    min_method="cellinterior",
    max_method="cellborder",
    sigma=5.0,
    verbose=False,
):
    """
